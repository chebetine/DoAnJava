					            ...
					            <div class="row">
					                <div class="col-md-12">
										<%
										    ...
										    // Nếu phương thức là GET, lấy thông tin từ CSDL để hiển thị
										    if (request.getMethod().equalsIgnoreCase("GET") && id != null) {
										        ...
										        try {
										            connection = KetNoi.getConnection();
										            String sql = "SELECT * FROM dsnhanvien WHERE idnhanvien = ?";
										            ...
										            if (resultSet.next()) {
										                ...
										                dob = resultSet.getString("ngaysinh");
										                gender = resultSet.getString("gioitinh");
										                hometown = resultSet.getString("quequan");
										                position = resultSet.getString("chucvu");
										                quyen = resultSet.getInt("quyentc");
										            }
										        } catch (SQLException e) {
										            e.printStackTrace();
										            out.println("<p>Lỗi lấy dữ liệu từ cơ sở dữ liệu: " + e.getMessage() + "</p>");
										        } finally {
										    ...
										    // Nếu phương thức là POST, xử lý cập nhật thông tin
										    if (request.getMethod().equalsIgnoreCase("POST")) {
										        ...
										        quyen = Integer.parseInt(request.getParameter("quyen"));
										
										        PreparedStatement preparedStatement = null;
										
										        try {
										            connection = KetNoi.getConnection();
										            String sqlUpdate = "UPDATE dsnhanvien SET hoten = ?, ngaysinh = ?, gioitinh = ?, quequan = ?, chucvu = ?, quyentc = ? WHERE idnhanvien = ?";
										            ...
										            preparedStatement.setInt(6, quyen);
										            ...
										            if (rowsUpdated > 0) {
										                ...
										                out.println("<p>Không tìm thấy nhân sự để cập nhật.</p>");
										            }
										        } catch (SQLException e) {
										            e.printStackTrace();
										...
										<form action="SuaNhanSu.jsp" method="post">
										    Họ tên: <input type="text" name="name" value="<%= name %>" required><br/><br/>
										    Ngày sinh: <input type="date" name="dob" value="<%= dob %>" required><br/><br/>
										    Giới tính:
										    <select id="levelSelect" name="gender">
										        <option value="Nam" <%= (gender.equals("Nam") ? "selected" : "") %>>Nam</option>
										        <option value="Nữ" <%= (gender.equals("Nữ") ? "selected" : "") %>>Nữ</option>
										    </select><br/><br/>
										    Địa chỉ: <input type="text" name="hometown" value="<%= hometown %>" required><br/><br/>
										    Chức vụ: <input type="text" name="position" value="<%= position %>" required><br/><br/>
										    Quyền truy cập: <input type="text" name="quyen" value="<%= quyen %>" required><br/><br/>
										    <input type="hidden" name="id" value="<%= id %>">
										    <input type="submit" name="submit" value="Lưu thông tin">
										</form>
										<%
								        if (request.getParameter("submit") != null) { 
								        	response.sendRedirect("DanhSachNV.jsp");
								        }
								        %>
					                </div>
					            </div>
