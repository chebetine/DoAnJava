		                ...
		                <div class="card">
						    <div class="card-header">
						        <div class="d-flex align-items-center flex-wrap">
						            <h4 class="card-title me-5">Yêu cầu của nhân viên</h4>
						            <form action="" method="get" class="d-flex align-items-center flex-nowrap">
						                <div class="form-group me-2">
						                    <label for="largeSelectYC">
						                        <select class="form-select form-control-lg" name="yeucau" id="largeSelectYC" style="min-width: 100px;">
						                            <option value="Tất cả" <%= "Tất cả".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Tất cả các yêu cầu</option>
						                            <option value="Nghỉ phép" <%= "Nghỉ phép".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Nghỉ phép</option>
						                            <option value="Làm thêm" <%= "Làm thêm".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Làm thêm</option>
						                            <option value="Thay đổi thông tin" <%= "Thay đổi thông tin".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Thay đổi thông tin hồ sơ</option>
						                            <option value="Ứng lương" <%= "Ứng lương".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Ứng lương</option>
						                            <option value="Khác" <%= "Khác".equals(request.getParameter("yeucau")) ? "selected" : "" %>>Yêu cầu khác</option>
						                        </select>
						                    </label>
						                </div>
						                <div class="ms-auto">
						                    <button type="submit" class="btn btn-primary btn-round">Xem</button>
						                </div>
						            </form>
						        </div>
						    </div>
						    <div class="card-body">
						        <div class="table-responsive">
						            <%
						                // Lấy giá trị của tham số "yeucau"
						                ...
						                try {
						                    ...
						                    if (connection != null) {
						                        ...
						                        // Kiểm tra xem "yeucau" có giá trị là "Tất cả" hoặc không có giá trị nào được truyền vào
						                        if (yeucau == null || yeucau.isEmpty() || "Tất cả".equals(yeucau)) {
						                            // Nếu không có yêu cầu cụ thể, hiển thị tất cả
						                            query = "SELECT nv.idnhanvien, nv.hoten, nv.chucvu, yc.LoaiYC, yc.NgayYC, yc.NgayBD, yc.NgayKT, yc.TrangThai, yc.MoTa, yc.PhanHoi "
						                                  + "FROM dsnhanvien nv "
						                                  + "JOIN qlyeucau yc ON yc.idnhanvien = nv.idnhanvien";
						                            preparedStatement = connection.prepareStatement(query);
						                        } else {
						                            // Nếu có loại yêu cầu, lọc theo loại yêu cầu
						                            query = "SELECT nv.idnhanvien, nv.hoten, nv.chucvu, yc.LoaiYC, yc.NgayYC, yc.NgayBD, yc.NgayKT, yc.TrangThai, yc.MoTa, yc.PhanHoi "
						                                  + "FROM dsnhanvien nv "
						                                  + "JOIN qlyeucau yc ON yc.idnhanvien = nv.idnhanvien "
						                                  + "WHERE yc.LoaiYC = ?";
						                            preparedStatement = connection.prepareStatement(query);
						                            preparedStatement.setString(1, yeucau);
						                        }
						
						                        resultSet = preparedStatement.executeQuery();
						
						                        out.println("<div>");
						                        out.println("<h6>Danh sách yêu cầu " + (yeucau == null || yeucau.isEmpty() || "Tất cả".equals(yeucau) ? "tất cả" : yeucau) + "</h6>");
						                        out.println("</div>");
						
						                        // Bắt đầu bảng
						                        out.println("<table id='add-row' class='display table table-striped table-hover'>");
						                        out.println("<thead><tr><th scope='col'>Họ tên</th><th scope='col'>Chức vụ</th><th scope='col'>Ngày gửi yêu cầu</th>"
						                                    + "<th scope='col'>Loại yêu cầu</th><th scope='col'>Ngày bắt đầu</th><th scope='col'>Ngày kết thúc</th>"
						                                    + "<th scope='col'>Trạng thái</th><th scope='col'>Mô tả</th><th scope='col'>Phản hồi</th><th scope='col' style='width: 10%'>Tùy chọn</th></tr></thead>");
						                        out.println("<tbody>");
						                        ...
						                        while (resultSet.next()) {
						                            ...
						                            String ngayyc = resultSet.getString("NgayYC");
						                            String loaiyc = resultSet.getString("LoaiYC");
						                            String ngaybd = resultSet.getString("NgayBD");
						                            String ngaykt = resultSet.getString("NgayKT");
						                            String trangthai = resultSet.getString("TrangThai");
						                            String mota = resultSet.getString("MoTa");
						                            String phanhoi = resultSet.getString("PhanHoi");
						
						                            out.println("<tr>");
						                            out.println("<td>" + ten + "</td>");
						                            out.println("<td>" + chucvu + "</td>");
						                            out.println("<td>" + ngayyc + "</td>");
						                            out.println("<td>" + loaiyc + "</td>");
						                            out.println("<td>" + ngaybd + "</td>");
						                            out.println("<td>" + ngaykt + "</td>");
						                            out.println("<td>" + trangthai + "</td>");
						                            out.println("<td>" + mota + "</td>");
						                            out.println("<td>" + phanhoi + "</td>");
						                            out.println("<td>");
						                            out.println("<div class='form-button-action'>");
						                            out.println("<button type='button' data-bs-toggle='tooltip' title='Sửa' class='btn btn-link btn-primary btn-lg' data-original-title='Edit Task'>");
						                            out.println("<a href='SuaNhanSu.jsp?id=" + idNhanvien + "' style='color:blue'><i class='fa fa-edit'></i></a>");
						                            out.println("</button>");
						                            out.println("<button type='button' data-bs-toggle='tooltip' title='Xóa' class='btn btn-link btn-danger' data-original-title='Remove'>");
						                            out.println("<a href='?deleteId=" + idNhanvien + "' onclick='return confirm(\"Bạn có chắc chắn muốn xóa không?\");' style='color:red'><i class='fa fa-times'></i></a>");
						                            out.println("</button>");
						                            out.println("</div>");
						                            out.println("</td>");
						                            out.println("</tr>");
						                        ...
						                        if (!hasData) {
						                            out.println("<tr><td colspan='10'>Không có dữ liệu yêu cầu</td></tr>");
						                        }
						                    ...
						                    e.printStackTrace();
						                    out.println("<p>Lỗi khi thực hiện truy vấn SQL: " + e.getMessage() + "</p>");
						                } finally {
						                    try {
						                        if (resultSet != null) resultSet.close();
						                        if (statement != null) statement.close();
