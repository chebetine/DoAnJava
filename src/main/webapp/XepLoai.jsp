...
<html lang="en">
	  <head>
	    ...
	    <title>Xếp loại nhân viên</title>
	    <meta
	          ...
	          <div class="page-inner">
		          ...
		          <div class="page-inner">
		            <div class="page-header">
		              <h3 class="fw-bold mb-3">Xếp loại nhân viên</h3>
		                ...
		                <div class="card">
						    ...
						    <div class="d-flex align-items-center flex-wrap">
						      ...
						      <form action="" method="get" class="d-flex align-items-center flex-nowrap">
						        <div class="form-group me-2">
						          <label for="largeSelectThang">Tháng: 
						  ...
						  <div class="card-body">
						    <div class="table-responsive">
						      <%
						      String thangChon = request.getParameter("thang");
						      String namChon = request.getParameter("nam");

						      try {
						          connection = KetNoi.getConnection();
						          if (connection != null) {
						        	  String query = "SELECT cc.idnhanvien, nv.hoten, nv.chucvu, cc.Thang, cc.Nam, xl.HieuSuat, xl.ThaiDo, xl.KyNang, cc.DiemTong, xl.XepLoai FROM dsnhanvien nv "
					                           + "JOIN dschamcong cc ON cc.idnhanvien = nv.idnhanvien "
			                        		   + "JOIN dsxeploai xl ON xl.MaCC = cc.MaCC "
					                           + "WHERE cc.Thang = ? AND cc.Nam = ?";
						              PreparedStatement preparedStatement = connection.prepareStatement(query);
						              preparedStatement.setString(1, thangChon);
						              preparedStatement.setString(2, namChon);
						              resultSet = preparedStatement.executeQuery();

						              out.println("<div>");
						              out.println("<h6>Tháng " + thangChon + " / Năm " + namChon + "</h6>");
						              out.println("</div>");

						              // Bắt đầu bảng
						              out.println("<table id='add-row' class='display table table-striped table-hover'>");
						              out.println("<thead><tr><th scope='col'>Họ tên</th><th scope='col'>Chức vụ</th><th scope='col'>Hiệu suất làm việc(%)</th>"
						                        + "<th scope='col'>Thái độ làm việc(%)</th><th scope='col'>Kỹ năng làm việc(%)</th><th scope='col'>Điểm chấm công(%)</th>"
						                        + "<th scope='col'>Xếp loại</th><th scope='col' style='width: 10%'>Tùy chọn</th></tr></thead>");
						              out.println("<tbody>");
						              ...
						              while (resultSet.next()) {
						                  ...
						                  int hieusuat = resultSet.getInt("HieuSuat");
						                  int thaido = resultSet.getInt("ThaiDo");
						                  int kynang = resultSet.getInt("KyNang");
						                  int diemtong = resultSet.getInt("DiemTong");
						                  String xeploai = resultSet.getString("XepLoai");

						                  out.println("<tr>");
						                  out.println("<td>" + ten + "</td>");
						                  out.println("<td>" + chucvu + "</td>");
						                  out.println("<td>" + hieusuat + "</td>");
						                  out.println("<td>" + thaido + "</td>");
						                  out.println("<td>" + kynang + "</td>");
						                  out.println("<td>" + diemtong + "</td>");
						                  out.println("<td>" + xeploai + "</td>");
						                  out.println("<td>");
						                  ...
						                  out.println("<a href='?deleteId=" + ten + "' onclick='return confirm(\"Bạn có chắc chắn muốn xóa không?\");' style='color:red'><i class='fa fa-times'></i></a>");
						                  out.println("</button>");
						                  out.println("</div>");
						                  out.println("</td>");
						                  out.println("</tr>");
						              ...
						              if (!hasData) {
						                  out.println("<tr><td colspan='8'>Không có dữ liệu cho tháng " + thangChon + " và năm " + namChon + "</td></tr>");
						              }
						  ...
						  %>
						    ...
						    <div class="ms-auto">
							  <a href="ThemXL.jsp" class="btn btn-primary btn-round">
							    Xếp loại tháng mới
							  </a>
							</div>
						  </div>
