		                   ...
		                   <%
							    ...
							    if (deleteId != null) {
							        try {
							            ...
							            if (connection != null) {
							                statement = connection.createStatement();
							                String sqlDelete = "DELETE FROM dsnhanvien WHERE hoten = '" + deleteId + "'"; // Cẩn thận với SQL Injection
							                statement.executeUpdate(sqlDelete);
							                out.println("<p>Đã xóa nhân viên: " + deleteId + "</p>");
							            }
							        } catch (SQLException e) {
							            e.printStackTrace();
							        } finally {
							    ...
							    try {
							        ...
							        if (connection != null) {
							            statement = connection.createStatement();
							            resultSet = statement.executeQuery("SELECT * FROM dsnhanvien");
							
							            // Bắt đầu bảng
							            out.println("<table id='add-row' class='display table table-striped table-hover'>");
							            out.println("<thead><tr><th scope='col'>Họ tên</th><th scope='col'>Ngày sinh</th><th scope='col'>Giới tính</th><th scope='col'>Quê quán</th><th scope='col'>Chức vụ</th><th scope='col' style='width: 10%'>Tùy chọn</th></tr></thead>"); // Thêm tiêu đề cho các cột
							            out.println("<tbody>");
							            while (resultSet.next()) {
							                ...
							                out.println("<td>"); // Bắt đầu cột hành động
							                ...
							                out.println("</button>");
							                out.println("</div>");
							                out.println("</td>"); // Kết thúc cột hành động
							                out.println("</tr>");
							            }
							            out.println("</tbody>");
							            // Kết thúc bảng
